...
for cost_names in cost_combinations:
    yeet = param_ranges.values()
    yeet2 = product(*param_ranges.values())
    for params in product(*param_ranges.values()):
        param_dict = dict(zip(param_ranges.keys(), params))

        # Construct cost expression
        cost_expr = sum(base_costs[name](param_dict[name]) for name in cost_names)
